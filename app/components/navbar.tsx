...
export function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (current > lastScrollY && current > 80 && !mobileMenuOpen) {
        setHidden(true);
      } else {
        setHidden(false);
      }
    ...
    <>
          ...
          <div className="flex items-center gap-4 text-[0.7rem] tracking-[0.3em] text-neutral-300 md:gap-6">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <button
                ...
                className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/25 md:hidden"
              >
      ...
      {mobileMenuOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-20 flex flex-col bg-black/95 px-6 pt-32 backdrop-blur-xl md:hidden"
        >
          <div className="flex flex-col gap-8 text-center text-xl font-light uppercase tracking-[0.2em] text-white">
            <button onClick={() => scrollToSection('lookbook')} className="py-2">
              Lookbook
            </button>
            <button onClick={() => scrollToSection('manifesto')} className="py-2">
              Manifesto
            </button>
            <button onClick={() => scrollToSection('visit')} className="py-2">
              Visit Us
            </button>
          </div>
